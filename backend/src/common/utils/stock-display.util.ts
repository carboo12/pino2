...
/**
 * Converts bulks and loose units into total base units.
 */
export function bulkUnitsToTotal(
  ...
  if (fallbackTotalQuantity !== undefined && Number.isFinite(fallbackTotalQuantity)) {
    return Math.max(0, Math.floor(fallbackTotalQuantity));
  }
...
/**
 * Splits a total quantity into bulks and loose units.
 */
