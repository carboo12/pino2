...
export default function PayablesPage() {
  ...
  const [selectedAccount, setSelectedAccount] = useState<AccountPayable | null>(
    null,
  );
  ...
  useEffect(() => {
    if (lastEvent && lastEvent.type !== "PING") loadData();
  }, [lastEvent]);

  const pendingTotal = useMemo(
    () => accounts.reduce((acc, a) => acc + Number(a.remainingAmount || 0), 0),
    [accounts],
  );
  const partialCount = useMemo(
    () => accounts.filter((a) => a.status === "PARTIAL").length,
    [accounts],
  );
  ...
  const submitPayment = async () => {
    ...
    try {
      ...
      toast.success(
        "Pago registrado",
        `Se abonaron ${formatCurrency(amount)} a ${selectedAccount.supplierName}.`,
      );
      resetPaymentDialog();
      await loadData(true);
    } catch (error: any) {
      ...
      toast.error(
        ...
        error?.response?.data?.message || "No se pudo procesar el abono.",
      );
    } finally {
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
        <div>
          ...
          <p className="text-muted-foreground">
            Control de saldos pendientes, deudas a proveedores y abonos
            registrados.
          </p>
        </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Facturas pendientes e historial de deuda activa.
          </CardDescription>
        </CardHeader>
        <CardContent>
          {loading ? (
            ...
            <Alert>
              ...
              <AlertDescription>
                No hay cuentas por pagar activas en tienda en este momento.
              </AlertDescription>
            </Alert>
          ) : (
              ...
              <Table>
                <TableHeader>
                  <TableRow>
                    ...
                    <TableHead className="text-right text-rose-600 font-bold">
                      Saldo Pendiente
                    </TableHead>
                ...
                <TableBody>
                  {accounts.map((acc) => (
                    <TableRow key={acc.id}>
                      ...
                      <TableCell
                        className={
                          acc.dueDate && new Date(acc.dueDate) < new Date()
                      ...
                      <TableCell className="text-right font-bold text-rose-600">
                        {formatCurrency(acc.remainingAmount)}
                      </TableCell>
                      <TableCell className="text-right">
                        <Button
                          size="sm"
      ...
      >
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              {selectedAccount
                ? `Saldo pendiente de ${selectedAccount.supplierName}: ${formatCurrency(selectedAccount.remainingAmount)}.`
                : ""}
            </DialogDescription>
          </DialogHeader>
